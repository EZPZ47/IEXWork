...
with st.form('population-form'):
    ...
    with col1:
        ...
        start_quarter = st.selectbox('Quarter',options=['Q1','Q2','Q3','Q4'],key='startqtr')
        start_year = st.slider('Year',min_value=1991,max_value=2023,step=1,value=1991)


    with col2:
        st.write('Choose an end date')
        end_quarter = st.selectbox('Quarter',options=['Q1','Q2','Q3','Q4'],key='endqtr')
        end_year = st.slider('Year',min_value=1991,max_value=2023,step=1,value=2023)

    
    with col3:
        st.write('Choose a location')
        target = st.selectbox('Choose a locaiton',options=df.columns[1:], index=0,key='location')
